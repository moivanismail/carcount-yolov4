...
# Inisialisasi model YOLO dengan backend Metal
net = cv2.dnn.readNet("yolov7.weights", "yolov7.cfg")
net.setPreferableBackend(cv2.dnn.DNN_BACKEND_DEFAULT)  # Backend default untuk Metal di macOS
net.setPreferableTarget(cv2.dnn.DNN_TARGET_CPU)  # Gunakan CPU karena CUDA tidak tersedia di Mac M1
...
def detect_objects(frame):
    ...
    blob = cv2.dnn.blobFromImage(cv2.UMat(frame), 0.00392, (416, 416), (0, 0, 0), True, crop=False)
    net.setInput(blob)
